/**
 * For testing the bahavior characteristic to Business Cards, namely:
 * - Increasing tokenId number, from 0 to 2500
 * - Setting up oracles, and disregarding previous ones
 * - Modifying the update price
 * - Minting a new card, changing name, swapping name, updateTokenURI, and the corresponding _updateTokenURI events
 * - Starting and pausing of the sale
 * - Name reserved behavior
 * - Fund withdrawal
 * - Setting baseURI, defaultURI
 * - Retrieving the tokenURI for minted, unprocessed, and non minted tokens
 */
...
const oracleCallbackTokenURI = 'QmUr63bgQq3VWW9XsVviDGAFwYEZVs9AFWsTd56T9xCQmf'
const altOracleCallbackTokenURI = 'QmUANYk2Bga9sQiqtr3Gfk1Q7Mw1pUG16GesLYstWyDzrr'
...
function shouldBehaveLikeBusinessCard (baseURI, defaultURI, owner, oracle, altOracle, buyer1, buyer2, buyer3, approved, operator) {
    ...
    context('with sale started', function () {
        ...
        describe('withdraw', function () {
            ...
            // TODO: https://ethereum.stackexchange.com/questions/52073/truffle-test-how-to-test-ether-transfer-balance-of-specific-account assert w this

            // TODO: revisit this big time, withdrawing not working properly
            /* it('can withdraw all the funds to owner', async function () {
                let starting_balance = new BN(await web3.eth.getBalance(owner))
                // Calling the withdrawal function as the owner costs gas and results in a balance gain, emptying the Card contract funds
                let card_contract_balance = new BN(await web3.eth.getBalance(await this.token.address))
                let tx_dict = await this.token.withdraw({ from: owner })
                console.log(tx_dict)
                let gas_used = new BN(web3.utils.toWei(tx_dict.receipt.gasUsed.toString(), 'gwei'))
                let ending_balance = new BN(await web3.eth.getBalance(owner))
                console.log(starting_balance.toString())
                console.log(ending_balance.toString())
                let balance_gain = ending_balance - starting_balance
                console.log(balance_gain.toString())
                console.log(card_contract_balance.toString())
                console.log(gas_used.toString())
                let expected_amount = card_contract_balance - gas_used
                console.log(expected_amount)
                // Balance in the contract should now be equal to zero
                card_contract_balance = await web3.eth.getBalance(await this.token.address)
                expect(card_contract_balance).to.be.equal('0')
                // Gain in balance should be equal to the funds that were held in the Card contract minus the gas spent for calling it
                expect(balance_gain).to.be.equal(expected_amount)
            }) */
        })
