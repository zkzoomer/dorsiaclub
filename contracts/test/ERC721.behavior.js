/**
 * Taken from: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/test/token/ERC721/ERC721.behavior.js
 * Adapted to reflect the BusinessCard behavior in minting new tokens.
 * Some tests have been taken out, either because the behavior is not allowed by the Business Card smart contract, or because
 * their ERC721 behavior is overwritten and tested in BusinessCard.test.js
 * 
 */
...
function shouldBehaveLikeERC721 (errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function () {
    beforeEach(async function () {
      await this.token.getCard(firstToken[0], firstToken[1], { from: owner, value: mintPrice});   
      await this.token.getCard(secondToken[0], secondToken[1], { from: owner, value: mintPrice});
    ...
    describe('transfers', function () {
      ...
      const transferWasSuccessful = function ({ owner, tokenId, approved }) {
        ...
        it('emits a Transfer event', async function () {
          expectEvent(tx, 'Transfer', { from: owner, to: this.toWhom, tokenId: tokenId });
        });
        ...
        it('emits an Approval event', async function () {
          expectEvent(tx, 'Approval', { owner, approved: ZERO_ADDRESS, tokenId: tokenId });
        });
      ...
      const shouldTransferTokensByUsers = function (transferFunction) {
        context('when called by the owner', function () {
          beforeEach(async function () {
            tx = await transferFunction.call(this, owner, this.toWhom, tokenId, { from: owner });
          });
        ...
        context('when called by the approved individual', function () {
          beforeEach(async function () {
            tx = await transferFunction.call(this, owner, this.toWhom, tokenId, { from: approved });
          });
          transferWasSuccessful({ owner, tokenId, approved });
        });
        ...
        context('when called by the operator', function () {
          beforeEach(async function () {
            tx = await transferFunction.call(this, owner, this.toWhom, tokenId, { from: operator });
          });
          transferWasSuccessful({ owner, tokenId, approved });
        });
        ...
        context('when sent to the owner', function () {
          beforeEach(async function () {
            tx = await transferFunction.call(this, owner, owner, tokenId, { from: owner });
          });
    ...
    // EXCLUDED TESTS: Contract does not let you to define a destination address when minting, gets sent to the tx sender
    ...
    describe('approve', function () {
      ...
      context('when clearing approval', function () {
        context('when there was no prior approval', function () {
          beforeEach(async function () {
            tx = await this.token.approve(ZERO_ADDRESS, tokenId, { from: owner });
          });
      ...
      context('when approving a non-zero address', function () {
        context('when there was no prior approval', function () {
          beforeEach(async function () {
            tx = await this.token.approve(approved, tokenId, { from: owner });
          });
        ...
        context('when there was a prior approval to a different address', function () {
          beforeEach(async function () {
            ...
            tx = await this.token.approve(anotherApproved, tokenId, { from: owner });
          });
      ...
      context('when the sender is an operator', function () {
        beforeEach(async function () {
          await this.token.setApprovalForAll(operator, true, { from: owner });
          tx = await this.token.approve(approved, tokenId, { from: operator });
        });
    ...
    describe('setApprovalForAll', function () {
      context('when the operator willing to approve is not the owner', function () {
        context('when there is no operator approval set by the sender', function () {
          ...
          it('emits an approval event', async function () {
            const tx = await this.token.setApprovalForAll(operator, true, { from: owner });
  ...
  describe('_mint(address, uint256)', function () {
    ...
    context('with minted token', async function () {
      beforeEach(async function () {
        tx = await this.token.getCard(firstToken[0], firstToken[1], { from: owner, value: mintPrice });
      });

      it('emits a Transfer event', function () {
        expectEvent(tx, 'Transfer', { from: ZERO_ADDRESS, to: owner, tokenId: firstTokenId });
      });
      ...
      // EXCLUDED TESTS: Contract does not let you define which tokenId you are minting, gets generated in order
    });
  });

  // EXCLUDED TESTS: Contract does not allow for the burning of tokens
}
...
function shouldBehaveLikeERC721Metadata (errorPrefix, name, symbol, owner) {
  ...
  describe('metadata', function () {
    ...
    // EXCLUDED TESTS: Token URI is tested inside BusinessCard.test.js, as its behavior is specific to the Business Card contract
  });
}
